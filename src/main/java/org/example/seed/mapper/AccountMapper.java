...
public interface AccountMapper {
    ...
    @Insert("INSERT INTO account (id, first_name, last_name, email, secret, deleted, create_date, change_date)" +
            "VALUES(#{chef.account.id}, #{chef.account.firstName}, #{chef.account.lastName}, #{chef.account.email}, NULL, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)")
    ...
    @Update("UPDATE account SET first_name = #{chef.account.firstName}, last_name = #{chef.account.lastName}, email = #{chef.account.email}, change_date = CURRENT_TIMESTAMP" +
            "WHERE account.id = #{chef.account.id} AND deleted = 0")
